...
counts = df[df['created_at'] >= cutoff].groupby(['date', 'hour', 'source']).size().reset_index(name='count')

# source毎の投稿数をカウントする
source_counts = df[df['created_at'] >= cutoff].groupby(['source']).size().reset_index(name='total_count')
