...
df = pd.read_csv('D:\\Python\\snscrape\\test.csv')
...
# ソースがnullの場合は"Unknown"として扱う
df.loc[df['source'].isnull(), 'source'] = "Unknown"
...
color_map = {'Twitter for iPhone': 'red', 'Twitter Web App': 'green', 'Twitter for Android': 'blue'}
counts['color'] = counts['source'].map(lambda x: color_map.get(x, random.choice(['purple', 'orange', 'yellow', 'pink'])))
